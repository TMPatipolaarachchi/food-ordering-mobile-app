...
export default function AdminCategoryScreen({ navigation }) {
  ...
  const uploadImage = async (uri) => {
    ...
    try {
      ...
      if (!response.ok) {
        console.log('Category upload failed:', response.status, responseText);
      ...
      console.log('Category upload failed:', error?.response?.status, error?.response?.data || error?.message);
